...
// Test Cases
TEST_F(TeleopTwistJoyTest, testHolonomicJoy)
{
  double cmd = 1;
  ...
  EXPECT_NEAR(cmd_vel.linear.y, cmd * scale_linear_map["y"], EPS);
  EXPECT_NEAR(cmd_vel.angular.z, cmd * scale_angular_map["yaw"], EPS);
}

TEST_F(TeleopTwistJoyTest, testHolonomicJoyNotEnabled)
{
  double cmd = 1;
  sensor_msgs::Joy cmd_joy;
  cmd_joy.buttons.resize(20);
...
TEST_F(TeleopTwistJoyTest, testHolonomicJoyTurbo)
{
  double cmd = 1;
  sensor_msgs::Joy cmd_joy;
  cmd_joy.buttons.resize(20);
  ...
  EXPECT_NEAR(cmd_vel.linear.y, cmd * scale_linear_turbo_map["y"], EPS);
  EXPECT_NEAR(cmd_vel.angular.z, cmd * scale_angular_map["yaw"], EPS);
}
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "holonomic_joy_test");
